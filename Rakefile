...
task :create_measure_csv do
  ...
  new_csv_file = 'C:/GitRepos/OpenStudio-PTool/local_measures.csv'
  FileUtils.rm_f(new_csv_file) if File.exist?(new_csv_file)
  csv = CSV.open(new_csv_file, 'w')
  Dir.glob('C:/GitRepos/OpenStudio-PTool/measures/**/measure.json').each do |file|
...
task :update_measure_jsons do
  ...
  Dir['C:/GitRepos/OpenStudio-PTool/measures/**/measure.rb'].each do |m|
    puts "Parsing #{m}"
