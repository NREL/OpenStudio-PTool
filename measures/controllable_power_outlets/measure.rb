...
# start the measure
class ControllablePowerOutlets < OpenStudio::Ruleset::ModelUserScript

  # human readable name
  def name
    return "Controllable Power Outlets"
  end
  ...
  def run(model, runner, user_arguments)
    ...
    model.getElectricEquipments.each do |equip|
      if equip.schedule.empty?
        ...
        runner.registerInfo("Schedule for the electric equipment object named '#{equip.name}' was reduced to simulate the application of Controllable Power Outlets.")
      end
    end
    ...
    runner.registerFinalCondition("#{original_equip_schs.uniq.size} schedule(s) were edited to reflect the addition of Controllable Power Outlets to the plug loads in the building.")
...
ControllablePowerOutlets.new.registerWithApplication
