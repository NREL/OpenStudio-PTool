  ...
  def run(model, runner, user_arguments)
	...
	thermal_zones.each do |thermal_zone|
		...
		if thermal_zone.thermostatSetpointDualSetpoint.is_initialized
			...
			# clone the existing schedule in case it is used somewhere else..
			@cloned_clg_tstat_schedule = @clg_tstat_schedule.clone
				#to do - name cloned schedule
	...
	if @thermal_zone_array.length == 0
		runner.registerAsNotApplicable("Measure is not applicable. There is no thermal zone in the model.")
	return true
	end
    ...
    runner.registerInitialCondition("The initial model contained #{@thermal_zone_array.length} 'Cooling Thermostat Schedule' & #{@thermal_zone_array.length} 'Heating Thermostat Schedule' objects for which this measure is applicable.")
