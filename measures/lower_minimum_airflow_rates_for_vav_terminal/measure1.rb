			...
			thermal_zones.each do |thermal_zone|
				...
				zone_equip.each do |vav_box|
					...
					if not airterminal_singleduct_vavreheat.empty?
						...
						runner.registerInfo("Minimum Airflow rate for Single duct VAV named '#{atsdvr}' with area #{ft2_thermal_zone_area} sqft, & zone minimum air flow input method as '#{existing_method}' = #{existing_cfm} #{existing_cons_air_frac} has been changed to a minimum fixed flow rate of #{cubic_mps} cfm.")
						...
						if not airterminal_singleduct_vavnoreheat.empty?
							...
							if existing_method_2 == "FixedFlowRate" 
								existing_cubic_mps_2 = airterminal_singleduct_vavnoreheat.fixedMinimumAirFlowRate
								existing_cfm_2 = OpenStudio.convert(existing_cubic_mps_2,"m^3/s","cfm")
								existing_cfm_2 = neat_numbers(existing_cfm_2,2)
							end
							if existing_method_2 == "Constant" 
							...
							runner.registerInfo("Minimum Airflow rate for Single duct VAV named '#{atsdvnr}' with area #{ft2_thermal_zone_area} sqft, & zone minimum air flow input method as '#{existing_method_2}' = #{existing_cfm_2} #{existing_cons_air_frac_2} has been changed to a minimum fixed flow rate of #{cubic_mps_2} cfm.")
							#increment counter of above object by one
