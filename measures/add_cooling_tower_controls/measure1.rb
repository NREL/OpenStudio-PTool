...
# start the measure
class AddCoolingTowerControls < OpenStudio::Ruleset::ModelUserScript
...
# Create array of all plant loops where plantLoop.SizingPlant.Looptype = "Condensor" 	
	...
	runner.registerInfo("Removed CoolingTowerSingleSpeed object named #{ss.name}.")
	runner.registerInfo("Adding new CoolingTowerVariableSpeed object named #{new_vs_cooling_tower.name} to replace #{ss.name}.")
	runner.registerInfo("Replacement Tower for\n #{ss.name} has same basin heater capacity of #{basin_heater_capacity} W.")
	runner.registerInfo("Replacement Tower for #{ss.name} has Design Inlet Air Wetbub Temperature of 25.56°C based on CTI testing standards.")
	runner.registerInfo("Replacement Tower for #{ss.name} has Design Range Temperature of 5.56°C based on CTI testing standards.")
	runner.registerInfo("Replacement Tower for #{ss.name} has Design Approach Temperature of 3.89°C based on CTI testing standards.")

	if design_water_flow_rate_autosize_status == true
		runner.registerInfo("Replacement Tower for #{ss.name} design water flow rate set to Autosize, same as the original cooling tower.")
	else
		runner.registerInfo("Replacement Tower for #{ss.name} design water flow rate set to #{design_water_flow_rate} m3/sec, same as the original cooling tower.")
	end
	
	if design_air_flow_rate_autosize_status == true
		runner.registerInfo("Replacement Tower for #{ss.name} resuses the design air flow rate setting of Autosize.")
	else
		runner.registerInfo("Replacement Tower for #{ss.name} reuses the design air flow rate of #{design_air_flow_rate} m3/sec.")
	end

	if fan_power_at_design_flow_rate_autosize_status == true
		runner.registerInfo("Replacement Tower for #{ss.name} resuses the Design Fan Power setting 'Autosize'.")
	else
		runner.registerInfo("Replacement Tower for #{ss.name} reuses the Design Fan Power setting of #{fan_power_at_design_air_flow_rate} W.")
	end
	
	runner.registerInfo("Replacement Tower for #{ss.name} uses a new performance curve named #{fan_pwr_func_airflow_ratio_curve.name} to describe the tower fan power vs airflow unloading curve.")

	runner.registerInfo("Replacement Tower for #{ss.name} sets the minimum tower airflow ratio to #{ss_min_airflow_ratio} of design airflow based on best engineering practice.")
	runner.registerInfo("Replacement Tower for #{ss.name} sets the fraction of tower capacity in free convection regime = #{ss_fract_twr_cap_free_con} based on induicstry standards.")
	runner.registerInfo("Replacement Tower for #{ss.name} reuses the basin heater setpoint temperature of #{basin_heater_setpoint_temp} Deg C.")

	if ss.basinHeaterOperatingSchedule.is_initialized
		runner.registerInfo("Replacement Tower for #{ss.name} reuses the Basin Heater Operating Schedule of #{basin_heater_operating_schedule}.")
	end

	if ss.evaporationLossMode.is_initialized
		runner.registerInfo("Replacement Tower for #{ss.name} reuses the evaporation loss mode setting of #{evaporation_loss_mode}.")
	end
		
	runner.registerInfo("Replacement Tower for #{ss.name} reuses the evaporation loss mode of #{evaporation_loss_mode}.")
	runner.registerInfo("Replacement Tower for #{ss.name} reuses the evaporation loss factor of #{evaporation_loss_factor}.")
	runner.registerInfo("Replacement Tower for #{ss.name} reuses the drift loss % of #{drift_loss_percent}.")

	if ss.blowdownCalculationMode.is_initialized
		runner.registerInfo("Replacement Tower for #{ss.name} reuses the blowdown calculation mode setting of #{blowdown_calculation_mode}.")
	end
	
	runner.registerInfo("Replacement Tower for #{ss.name} reuses the blowdown concentration ratio of #{blowdown_concentration_ratio}.")
	
	if ss.blowdownMakeupWaterUsageSchedule.is_initialized
		runner.registerInfo("Replacement Tower for #{ss.name} reuses the blowdown makeup water usage schedule named #{blowdown_makeup_water_usage_schedule}.")
	end

	runner.registerInfo("Replacement Tower for #{ss.name} reuses the same number of cells of the original tower, #{number_of_cells} cells.")
	runner.registerInfo("Replacement Tower for #{ss.name} reuses the cell control strategy of #{cell_control}.")
	runner.registerInfo("Replacement Tower for #{ss.name} reuses the cell minimum water flow rate fraction of #{cell_minimum_water_flow_rate_fraction}.")
	runner.registerInfo("Replacement Tower for #{ss.name} reuses the cell minimum water flow rate fraction of #{cell_maximum_water_flow_rate_fraction}.")
	runner.registerInfo("Replacement Tower for #{ss.name} reuses the sizing factor of #{sizing_factor}.")
...
AddCoolingTowerControls.new.registerWithApplication
