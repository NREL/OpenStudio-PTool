...
class StaticPressureReset < OpenStudio::Ruleset::ModelUserScript
  ...
  def run(model, runner, user_arguments)
    ...
    runner.registerInitialCondition("The building has #{results.length} variable air volume units for which this measure is applicable.")

    #reporting final condition of model
    runner.registerFinalCondition("Static pressure controls were applied to  #{results.length} multi-zone, variable air volume units in the model.  Airloops affected were #{airloop_name}")
    return true
