...
# start the measure
class PCNetworkPresence < OpenStudio::Ruleset::ModelUserScript

  # human readable name
  def name
    return "PC Network Presence"
  end
  ...
  def run(model, runner, user_arguments)
    ...
    model.getElectricEquipments.each do |equip|
      if equip.schedule.empty?
        ...
        runner.registerInfo("Schedule for the electric equipment object named '#{equip.name}' was reduced to simulate the application of PC Network Presence.")
      end
    end
    ...
    runner.registerFinalCondition("#{original_equip_schs.uniq.size} schedule(s) were edited to reflect the addition of PC Network Presence to the computers in the building.")
...
PCNetworkPresence.new.registerWithApplication
