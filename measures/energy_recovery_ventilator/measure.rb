...
# start the measure
class EnergyRecoveryVentilator < OpenStudio::Ruleset::ModelUserScript
  ...
  def description
    return "Energy recovery ventilators (ERVs) transfer some of the energy between the entering outdoor air and the leaving exhaust air.  This lowers the amount of conditioning that needs to be performed on the entering outdoor air, saving energy.  However, ERVs add additional an fan pressure rise to the HVAC system, and the increased fan energy may outweigh the saving in heating and cooling energy in milder climates."
  end

  # human readable description of modeling approach
  def modeler_description
    return "For each AirLoop, add a rotary ERV with a user-specified increase in fan pressure rise.  This measure currently assumes that the ERV is always creating an additional pressure rise, even when it is being bypassed.  This measure also assumes that the ERV rotation motor consumes a negligible amount of energy."
  end
...
EnergyRecoveryVentilator.new.registerWithApplication
