...
class AdvancedRTUControls < OpenStudio::Ruleset::WorkspaceUserScript
  ...
  def name
    return "Advanced RTU Controls"
  end

  # human readable description
  def description
    return "This energy efficiency measure (EEM) adds a variable speed drive (VSD) to existing constant-speed, single-stage cooling rooftop units (RTUs). The VSD reduces the fan speed when the RTU is in economizer or ventilation mode, saving energy compared to a constant-speed fan baseline where the fan is run at 100% flow regardless of need."
  end
