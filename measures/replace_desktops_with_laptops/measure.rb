#start the measure
class ReplaceDesktopsWithLaptops < OpenStudio::Ruleset::ModelUserScript

  #define the name that a user will see
  def name
    return "Replace Desktops with Laptops"
  end
  ...
  def run(model, runner, user_arguments)
    ...
    frac_desktop = 0.53
    other_power_w = 40 # laptop
    runner.registerInfo("Assume that each laptop uses #{other_power_w.round}W at peak.")
    frac_other = 0.47
    ...
    # Calculate the number of desktops currently in the building
    runner.registerFinalCondition("After replacing #{num_desktops.round} #{desktop_power_w.round}W desktops with #{other_power_w.round}W laptops, there are now #{final_equip_power_w.round} W of electric equipment installed in the building.")
...
ReplaceDesktopsWithLaptops.new.registerWithApplication