...
class ElectrochromicWindowsTests < MiniTest::Unit::TestCase
  ...
  def test_sec_schl_2010_2A
    ...
    # load the test model
    translator = OpenStudio::OSVersion::VersionTranslator.new
    path = OpenStudio::Path.new(File.dirname(__FILE__) + "/SecondarySchool-90.1-2010-ASHRAE 169-2006-2A.osm")
    model = translator.loadModel(path)
    assert(model.is_initialized)
    model = model.get
    runner.setLastOpenStudioModel(model)
    ...
    workspace.save("#{Dir.pwd}/output/SecondarySchool-90.1-2010-ASHRAE 169-2006-2A.idf",true)
    
    assert(result.value.valueName == "Success")

  end    
   
  def test_sec_schl_1980_2004_3B
    ...
    workspace.save("#{Dir.pwd}/output/SecondarySchool-DOE Ref 1980-2004-ASHRAE 169-2006-3B.idf",true)
    
    assert(result.value.valueName == "Success")

  end
  
  def test_sec_schl_pre1980_3B
    ...
    # load the test model
    translator = OpenStudio::OSVersion::VersionTranslator.new
    path = OpenStudio::Path.new(File.dirname(__FILE__) + "/SecondarySchool-DOE Ref Pre-1980-ASHRAE 169-2006-3B.osm")
    model = translator.loadModel(path)
    assert(model.is_initialized)
    model = model.get
    runner.setLastOpenStudioModel(model)
    ...
    workspace.save("#{Dir.pwd}/output/SecondarySchool-DOE Ref Pre-1980-ASHRAE 169-2006-3B.idf",true)
