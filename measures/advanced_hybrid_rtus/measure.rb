#start the measure
class AdvancedHybridRTUs < OpenStudio::Ruleset::ModelUserScript
  ...
  def modeler_description
    return "Each packaged single zone system in the model will be replaced with an advanced hybrid RTU consisting of a 100% effective indirect evaporative cooler, a two speed DX cooling coil, the existing heating coil, and a variable speed fan.  Fan power will be increased by 0.5 inches of water column above the current fan pressure rise to represent the addition of the indirect evaporative cooler."
  end  
  ...
  def run(model, runner, user_arguments)
    ...
    # Find all packaged single zone systems
    air_loops_modified = []
    model.getAirLoopHVACs.each do |air_loop|
    
      # Skip multi-zone systems
    ...
    if air_loops_modified.size == 0
      runner.registerAsNotApplicable("Not Applicable.  No packaged single zone systems could be found to replace.")
    else
...
AdvancedHybridRTUs.new.registerWithApplication