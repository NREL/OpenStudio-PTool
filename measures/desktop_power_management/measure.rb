...
# start the measure
class DesktopPowerManagement < OpenStudio::Ruleset::ModelUserScript

  # human readable name
  def name
    return "Desktop Power Management"
  end
  ...
  def run(model, runner, user_arguments)
    ...
    model.getElectricEquipments.each do |equip|
      if equip.schedule.empty?
        ...
        runner.registerInfo("Schedule for the electric equipment object named '#{equip.name}' was reduced to simulate the application of Desktop Power Management.")
      end
    end
    ...
    runner.registerFinalCondition("#{original_equip_schs.uniq.size} schedule(s) were edited to reflect the addition of Desktop Power Management to the computers in the building.")
...
DesktopPowerManagement.new.registerWithApplication
