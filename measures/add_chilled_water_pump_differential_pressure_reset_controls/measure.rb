...
# start the measure
class AddChilledWaterPumpDifferentialPressureResetControls < OpenStudio::Ruleset::ModelUserScript
  ...
  # human readable name
  def name
    return " Add Chilled Water Pump Differential Pressure Reset Controls"
  end
  ...
  def run(model, runner, user_arguments)
	...
	model.getPlantLoops.each do |plantLoop|
		...
		if 	plantLoop.commonPipeSimulation.is_initialized
			common_pipe_simulation_status = plantLoop.commonPipeSimulation.get
			if loop_type == "Cooling" && common_pipe_simulation_status == "CommonPipe"
				qualified_cooling_plant_loop_array << plantLoop
			end
		else
			runner.registerWarning("Setting for the attribute 'Common Pipe Simulation' of plant loop named #{plantLoop.name} was blank. The measure will not change this plant loop object.")
		end # end if statement for plantLoop.commonPipeSimulation.is_initialized 
	...
	qualified_cooling_plant_loop_array.each do |qualified_loop|
		...
		qualified_loop.demandComponents.each do |dc|
			...
			if dc.to_PumpVariableSpeed.is_initialized
				...
				if @vs_pump_autosize_status == false 
					...
					vs_hardsize = vs_hardsize + 1
											
				end # end vs pump autosize status = false 
		...
		if qualified_cooling_plant_loop_array.size == 0 			
			runner.registerAsNotApplicable("The model does not contain any chilled water loops with a plantLoop.commonPipeSimulation setting = 'Common Pipe'. The measure is not applicible.")
			return true
		end
	
	end # end loop through qualified plant loops
...
AddChilledWaterPumpDifferentialPressureResetControls.new.registerWithApplication
